...
package body flash_program is
   ...
   begin
      if sub_cmd_list.Element (0) = "" then
         ...
         IO.Put_Line ("Too few arguments for the flash command. Run " &
         utilities_cli.bold & "help flash" & utilities_cli.unbold &
         " for required arguments");
      elsif sub_cmd_list.Length = 2 then
         flash_handler (sub_cmd_list.Element (0), sub_cmd_list.Element (1));
      else
         IO.Put_Line ("Too many arguments for the flash command. Run "
         & utilities_cli.bold & "help flash" & utilities_cli.unbold &
         " for required arguments");
      end if;
   end parse_sub_command;

   procedure default_flash_handler (file : in out Unbounded_String;
                                 address : in out Unbounded_String) is
   begin
      IO.Put (Ada.Characters.Latin_1.LF & utilities_cli.bold &
      "Flash Program" & utilities_cli.unbold & Ada.Characters.Latin_1.LF &
      Ada.Characters.Latin_1.LF);
      IO.Put ("Enter the file path: ");
      file :=
         Ada.Strings.Unbounded.To_Unbounded_String (Ada.Text_IO.Get_Line);
      IO.Put ("Enter the address of the flash" &
               "location in hexidecimal format: 0x");
      address :=
         Ada.Strings.Unbounded.To_Unbounded_String (Ada.Text_IO.Get_Line);
   end default_flash_handler;

   procedure flash_handler (file : Unbounded_String;
   ...
   procedure flash (file : Unbounded_String; address : Unbounded_String) is
      ...
      --  Address we will start writing to on the board should be a
   ...
   begin
      ...
      then
         ...
         IO.Put_Line (LF & "Flashing failed." & LF);
         erase_program.erase (Begin_Sector, End_Sector);
      end if;
      Status_Buffer (Ada.Streams.Stream_Element_Offset (1)) := 0;
